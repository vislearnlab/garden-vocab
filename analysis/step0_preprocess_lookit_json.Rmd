...
```{r}
lookit_json = here::here('data/garden/lookit/Visual-Vocabulary--A-picture-matching-game-_all-responses-identifiable_072924_final.json')
...
# all_children=c('response_id','start_time','child','parent')
...
```{r}
save(all_children, file = paste0(here::here('data/garden/lookit/all_children_identifiable'), today(), '.RData'))
```

```{r}
age_and_id <- all_children %>%
  select(child_id, completed, age_in_days) %>%
  mutate(age_in_years = age_in_days/365) %>%
  distinct(child_id, age_in_years)


write_csv(age_and_id, file = "garden_age_ids.csv")
```
